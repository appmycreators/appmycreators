...
import { useUpdateGalleryComment } from "@/hooks/useGalleryComments";
import type { GalleryComment } from "@/hooks/useGalleryComments";
...
export default function EditCommentModal({ 
  ...
  // Carregar dados do comentário ao abrir
  useEffect(() => {
    if (open && comment) {
      setAuthorName(comment.author_name);
      setCommentText(comment.comment_text);
      setAvatarUrl(comment.author_avatar_url || "");
    }
  }, [open, comment]);
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button
              type="submit"
              disabled={updateComment.isPending || uploading}
              className="flex-1"
            >
